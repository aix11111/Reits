...
def latest_metrics(df, code):
    """返回指定 code 最新一季度（按 period 字符串排序取最后一行）的指标摘要。

    返回 dict，键为 period（str）、noi_margin、net_margin、distributable_yield（float）。
    该 code 无数据时返回空 dict {}。
    """
    sub = df[df["code"] == code]
    if sub.empty:
    ...
    return {
        ...
        "distributable_yield": float(
            latest["distributable_wan"] * 4 / latest["nav_wan"]
        ),
    }
